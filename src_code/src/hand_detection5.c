...
#include <def_val.h>
void hands_detection5(unsigned int *ii_m, 
					unsigned short xmin, 
					unsigned short xmax,
					unsigned short ymin,
					unsigned short ymax)
	...
	for(i = 0; i < itt; i++)
	{
		...
		for(wy = ymin; wy < ymax - h; wy += (Scale >> 1))
		{
			for(wx = xmin; wx < xmax - w; wx += (Scale >> 1))
			{
				...
				{
					sum_stage = 0;
					for(i_trees = 0; i_trees < stage5[i_stage]; i_trees++)
					{
						Rectangle_sum = 0;
						for(i_feature = 0; i_feature < 3; i_feature++)
						{
							RectX = ((Rectx5[curr_feature_index]*Scale) >> 1) + wx;
							RectY = ((Recty5[curr_feature_index]*Scale) >> 1) + wy;
							RectW = (Rectw5[curr_feature_index]*Scale) >> 1;
							RectH = (Recth5[curr_feature_index]*Scale) >> 1;
							RectWeight = Rectweight5[curr_feature_index];
						...
						if(Rectangle_sum < threshold5[curr_threshold_index])
							sum_stage += left_val5[curr_threshold_index];
						else
							sum_stage += right_val5[curr_threshold_index];
						curr_threshold_index++;
					}
					if(sum_stage > stage_threshold5[i_stage])
						continue;
