			...
			for(wx = xmin; wx < xmax - w; wx += (Scale >> 1))
			{
				...
				for(i_stage = 0; i_stage < 20; i_stage++)
				{
					...
					for(i_trees = 0; i_trees < stageA[i_stage]; i_trees++)
					{
						...
						{
							RectX = ((RectxA[curr_feature_index]*Scale) >> 1) + wx;
							RectY = ((RectyA[curr_feature_index]*Scale) >> 1) + wy;
							RectW = (RectwA[curr_feature_index]*Scale) >> 1;
							RectH = (RecthA[curr_feature_index]*Scale) >> 1;
